...
RSpec.describe PolicyOcr::Parser::ParsePolicyDocumentText do
  describe ".call" do
    ...
    context "with invalid inputs" do
      it "fails when raw_text is nil" do
        result = PolicyOcr::Parser::ParsePolicyDocumentText.call(raw_text: nil)
        
        expect(result).to be_failure
        expect(result.error).to eq("raw_text is required")
      end

      it "fails when raw_text is empty" do
      ...
      it "fails when raw_text is blank" do
        result = PolicyOcr::Parser::ParsePolicyDocumentText.call(raw_text: "   \n\t  ")
